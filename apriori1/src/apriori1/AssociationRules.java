...
//        String[] inputText = {"banana apple pear milk", "milk banana apple", "magazine pencil paper", "paper pencil banana"};
        try {
            BufferedReader reader = new BufferedReader( new FileReader( "F:\\SEMESTER7\\FYP\\data_t2.csv" ) );
            try {                
                ...
                Logger.getLogger(AssociationRules.class.getName()).log(Level.SEVERE, null, ex);
            }
        } catch (FileNotFoundException ex) {
            Logger.getLogger(AssociationRules.class.getName()).log(Level.SEVERE, null, ex);
        }
        
        String[] inputText = (String[])text.toArray(new String[0]);
...
//            String[] thisBasket = basket[i].split("[\\s,]+");
            String[] thisBasket = basket[i].split(",");
